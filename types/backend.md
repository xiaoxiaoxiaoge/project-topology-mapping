...
```bash
# 1. Java 源码
git ls-files 'src/main/java/**/*.java' 2>/dev/null | head -100
...
git ls-files 'src/**/*Controller.java' 2>/dev/null | head -30
git ls-files 'src/**/*Service.java' 2>/dev/null | head -30
git ls-files 'src/**/*Repository.java' 2>/dev/null | head -30
```
...
# 2. Service 方法签名
git ls-files 'src/**/*Service.java' 2>/dev/null | head -30
git grep -h "public.*(" -- 'src/**/*Service.java' 2>/dev/null | head -50

# 3. Entity / Model / DO
git ls-files 'src/**/*Entity.java' 'src/**/*Model.java' 'src/**/*DO.java' 2>/dev/null | head -20
...
git grep -h "@FeignClient\|Feign\|fallback" -- '*.java' 2>/dev/null | head -30

# 10. 微服务调用（按服务名前缀）
git grep -h "GwBIZFeign\|GwDocsyFeign\|GwPicsyFeign\|GwVideosyFeign" -- '*.java' 2>/dev/null | head -20
```
...
```bash
# 1. Go 源码
git ls-files '*.go' 2>/dev/null | grep -v vendor | head -100
...
git grep -h "HandleFunc\|Handle(\|r\.Get\|r\.Post\|r\.Put\|r\.Delete\|router\." \
  -- '*.go' 2>/dev/null | grep -v vendor | head -30

# 3. 中间件
git grep -h "Use\|Middleware\|func.*Middleware" -- '*.go' 2>/dev/null | grep -v vendor | head -20

# 4. 数据库模型
git ls-files '*.go' 2>/dev/null | xargs grep -l "type.*struct" 2>/dev/null | grep -v vendor | head -20

# 5. ORM（GORM / XORM / Ent）
git grep -h "gorm\.\|xorm\.\|ent\." -- '*.go' 2>/dev/null | head -20

# 6. 配置（Viper / envconfig）
git grep -h "viper\.\|envconfig\|os\.Getenv" -- '*.go' 2>/dev/null | sort -u | head -20
```
...
```bash
# 1. 源码
git ls-files '*.py' 2>/dev/null | grep -v venv | grep -v __pycache__ | head -100
...
git grep -h "path\|url" -- 'urls.py' 2>/dev/null | head -30

# 3. Model
git ls-files 'models.py' 'models/*.py' 2>/dev/null
git grep -h "class.*Model\|class.*Admin" -- 'models.py' 'models/*.py' 2>/dev/null | head -20
...
git grep -h "Blueprint(" -- '*.py' 2>/dev/null | head -20

# 3. 装饰器路由
git grep -h "@app.route\|@bp.route" -- '*.py' 2>/dev/null | sort -u | head -30
```
...
# 2. 路由
git grep -h "@app\.\(get\|post\|put\|delete\|patch\)\|@router\." -- '*.py' 2>/dev/null | sort -u | head -30

# 3. 依赖注入
git grep -h "Depends(" -- '*.py' 2>/dev/null | sort -u | head -20

# 4. Pydantic 模型
git ls-files 'schemas/*.py' '*schema*.py' 2>/dev/null
git grep -h "BaseModel" -- '*.py' 2>/dev/null | head -20
```
...
```bash
# 1. 源码
git ls-files 'src/**/*.ts' 'src/**/*.js' 2>/dev/null | head -100
...
```bash
git grep -h "express\|@nestjs/core\|fastify\|koa\b\|hapi\b" -- 'package.json' 2>/dev/null | head -3
```
...
git grep -h "router\.\(get\|post\|put\|delete\|patch\)\|app\.\(get\|post\|put\|delete\|patch\)\|@Get\|@Post\|@Put\|@Delete" \
  -- '*.ts' '*.js' 2>/dev/null | sort -u | head -50
...
# 7. 异常过滤器
git grep -h "@Catch\|@UseFilters\|HttpException" -- '*.ts' 2>/dev/null | sort -u | head -20
```
...
```bash
# 1. HTTP 客户端
git grep -h "axios\|got\|node-fetch\|undici" -- '*.ts' '*.js' 2>/dev/null | sort -u | head -20

# 2. 拦截器（响应/请求）
git ls-files 'src/**/*interceptor*.ts' 2>/dev/null
git grep -h "intercept(\|@UseInterceptors" -- '*.ts' 2>/dev/null | sort -u | head -20

# 3. 缓存（ioredis / cache-manager）
git grep -h "ioredis\|cache-manager\|@nestjs/cache" -- '*.ts' 2>/dev/null | sort -u | head -20

# 4. 会话 / 鉴权
git grep -h "express-session\|@nestjs/jwt\|passport\|@UseGuards" -- '*.ts' 2>/dev/null | sort -u | head -20
```
...
```bash
# 1. C# 源码
git ls-files '*.cs' 2>/dev/null | head -100
...
# 4. EF Core / Dapper
git grep -h "DbContext\|DbSet\|EntityFramework\|IDbConnection" -- '*.cs' 2>/dev/null | sort -u | head -20

# 5. 中间件
git grep -h "UseMiddleware\|app\.Use" -- '*.cs' 2>/dev/null | sort -u | head -20

# 6. 鉴权
git grep -h "AddAuthentication\|AddAuthorization\|\[Authorize\]" -- '*.cs' 2>/dev/null | sort -u

# 7. SignalR
git grep -h "Hub\b\|IHubContext" -- '*.cs' 2>/dev/null | sort -u | head -20
...
git grep -h "HttpClient\|IHttpClientFactory" -- '*.cs' 2>/dev/null | sort -u | head -20
...
```bash
# 1. 源码
git ls-files 'src/*.rs' 2>/dev/null | head -50
...
```bash
git grep -h "actix-web\|axum\|rocket\|warp" -- 'Cargo.toml' 2>/dev/null | head -3
```
...
git grep -h "#\[get\|#\[post\|#\[put\|#\[delete\|#\[handler\|#\[route\|#\[actix_web::" \
  -- '*.rs' 2>/dev/null | sort -u | head -30

# 2. 路由注册
git grep -h "\.route(\|\.service(\|web::resource\|web::scope" -- '*.rs' 2>/dev/null | sort -u | head -20

# 3. 中间件
git grep -h "middleware\|wrap_fn\|from_fn" -- '*.rs' 2>/dev/null | sort -u | head -20

# 4. Trait 实现
git grep -h "impl.*for\|trait " -- '*.rs' 2>/dev/null | sort -u | head -20

# 5. 数据库（sqlx / diesel / sea-orm）
git grep -h "sqlx::\|diesel::\|sea_orm::" -- '*.rs' 2>/dev/null | sort -u | head -20

# 6. 错误处理
git grep -h "thiserror\|anyhow\|#\[derive(Debug" -- '*.rs' 2>/dev/null | sort -u | head -20
...
```bash
# 1. 状态共享
git grep -h "Arc<\|Mutex<\|RwLock<\|web::Data" -- '*.rs' 2>/dev/null | sort -u | head -20

# 2. HTTP 客户端（reqwest）
git grep -h "reqwest::" -- '*.rs' 2>/dev/null | sort -u | head -20

# 3. 配置
git grep -h "config::\|env::var\|Figment" -- '*.rs' 2>/dev/null | sort -u | head -20
```
...
# 高度变异：必须逐个采样
cat src/main/java/com/example/controller/FileFlowController.java
cat src/main/java/com/example/controller/PcScreenController.java
```
...
当项目涉及 Java 微服务调用 Go / Node / 其他语言服务时，容易犯的错误是：根据命名相似性推断服务关系，导致拓扑错误。
...
```bash
# Java
git grep -h "SERVICE_NAME.*=\|ServiceName.*=" -- '*.java' 2>/dev/null | grep -E "T1SDK|GW-"
...
**步骤 2：找到 @FeignClient / gRPC Client / HTTP Client 的 name 值**
...
git grep -h "server_name:" -- '*.yaml' '*.yml' '*.toml' 2>/dev/null
git grep -h "eureka\|Register\|ServerName" -- '*.go' 2>/dev/null | grep -v vendor | head -20
...
| Java @FeignClient name | Java 常量定义 | Go Eureka 注册名 | 结论 |
|------------------------|--------------|-----------------|------|
...
**规则：**
- 如果 `@FeignClient(name = "X")` 与目标服务 `server_name` 完全一致 → 同一服务
- 如果 `@FeignClient(name = "X")` 与目标服务 `server_name` 不同（如 `T1SDK-XXX` vs `GW-XXX`）→ **不同服务**，在拓扑中分开描述
...
```bash
# 1. 查找服务名常量定义文件
find . -name "GwServiceNameConstant*" -o -name "*ServiceNameConstant*" 2>/dev/null | head -5

# 2. 读取常量文件，列出所有服务名
cat src/.../GwServiceNameConstant.java 2>/dev/null | grep "public static final String.*SERVICE_NAME"

# 3. 列出所有 Feign Client 的服务名
git ls-files 'src/**/*Feign.java' 2>/dev/null | xargs -I {} sh -c 'echo "=== {} ==="; grep "@FeignClient" {}'

# 4. 查找目标服务的 server_name 配置
